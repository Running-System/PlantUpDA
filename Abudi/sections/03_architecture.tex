...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.45\textheight,keepaspectratio]{Abudi/figures/plantup_fig1_master_v2.png}
    \caption{Master Architecture illustrating the chronological data pipeline: sensors, ESP32, Wi-Fi/MQTT, Cloud Ingestion, unified PostgreSQL database, Edge Functions/AI, and the React Native app.}
...
Positioned across the cloud boundary, this service acts as a logical ingestion gateway, rather than relying on a physical local gateway hub (such as a Raspberry Pi). It operates independently of user-facing services and serves as the strict security protocol barrier between the external hardware ecosystem and the protected backend environment. 

By absorbing high-velocity MQTT payloads, this service translates hardware telemetry---including connectivity states, registration data, and sensor measurements---into consistent database entries. This abstraction protects the frontend application from underlying protocol complexities and isolates any hardware-level network issues from impacting user-facing application layers.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.45\textheight,keepaspectratio]{Abudi/figures/plantup_fig2_cloud_v2.png}
    \caption{Cloud ingestion boundary focusing on internal services, including authentication, Row-Level Security, and payload normalization within the Supabase environment.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth,height=0.45\textheight,keepaspectratio]{Abudi/figures/plantup_fig3_persistence_v3.png}
    \caption{Persistence Layer illustrating database schemas. TimescaleDB is used for scalable telemetry management in the Micro schema, while relational application states reside in the Social schema.}
