...
Following the experimental methodology established in Section 3.4, this chapter presents the empirical results of the performance evaluation. The data illustrates the latency, scalability, and structural resilience of the \textit{Thin Edge} architecture within the Plant Up! ecosystem.

\subsection{End-to-End Latency Benchmark}

To evaluate the real-time responsiveness of the system, an end-to-end latency benchmark was conducted to measure the interval between a physical sensor trigger and its final persistence in the cloud database. As the MQTT protocol operates asynchronously, these measurements were derived using the differential timestamping method described in Section 3.4.3.

The results indicate that while the cloud-centric approach introduces multiple network hops, the total response time remains well within the acceptable boundaries established for botanical monitoring in Section 3.1.3.
...
\begin{table}[htbp]
    ...
    \end{tabular}
    \caption{End-to-End Ingestion Latency (Sensors to Persistence). Source: Own work.}
    \label{tab:end_to_end_latency}
\end{table}

While this duration is higher than localized edge processing (estimated at approximately 100\,ms), it provides the foundational infrastructure required for the resource-heavy AI and social features that define the Plant Up! experience. As noted in Section 3.1.3, the biological constraints of houseplants make this latency trade-off negligible for operational health monitoring.

\subsection{Database Performance: TimescaleDB vs. Postgres}

Following the comparative methodology defined in Scenario 1 (see Section 3.4.3.1), an execution analysis was performed to quantify the performance gain of the TimescaleDB hypertable implementation. The empirical results, representing the average execution times across the benchmarking iterations, are summarized in Table \ref{tab:timescaledb_performance}.
...
\begin{table}[htbp]
    ...
    \end{tabular}
    \caption{Query Performance Comparison for Time-Series Data (500k rows, 100 runs). Source: Own work.}
...
To evaluate the scalability and stability of the ingestion pipeline, the infrastructure was subjected to the ``Disaster Recovery'' spike simulation defined in Scenario 2 (see Section 3.4.3.2). This stress test monitors the middleware's ability to handle high-velocity telemetry influxes without data loss or significant architectural degradation.

Under a load of 10,000 consecutive writes, the system successfully persisted all payloads with a 0\% drop rate, achieving a peak ingestion throughput of approximately 850 writes/sec. This benchmark proves that the MQTT gateway's concurrent queuing and batch processing logic effectively limits the frequency of outbound HTTP requests. By pooling the insertion requests, the system prevents hitting the cloud provider's API rate limits and significantly reduces network connection handshake overhead.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{} l l l @{}}
        ...
        \textbf{Latency Benchmark} & $\approx$100\,ms (Estimated) & 264\,ms (Empirical) \\ \bottomrule
    \end{tabular}
    \caption{Contextual Analysis of Measured Thin Edge Performance vs. Localized Baselines. Source: Own work.}
